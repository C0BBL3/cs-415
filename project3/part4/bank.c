...
void initialize_shared_memory() 
{
    ...
    shm_fd = shm_open("/account_shared_memory", O_CREAT | O_RDWR, 0666);
    ftruncate(shm_fd, sizeof(shared_account) * num_accounts);
    shared_accounts = mmap(NULL, sizeof(shared_account) * num_accounts, PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0);

    // Savings account initialization
    for (int i = 0; i < num_accounts; i++) {

        // Initialize Puddles Bank account in shared memory
        strcpy(shared_accounts[i].account_number, accounts[i].account_number);
        shared_accounts[i].duck_bank_balance = accounts[i].balance;

        // Initialize Puddles Bank savings (20% of Duck Bank balance)
        shared_accounts[i].puddles_bank_balance = shared_accounts[i].duck_bank_balance * 0.2;

        shared_accounts[i].puddles_bank_balance = 0.2 * accounts[i].balance; // 20% of Duck Bank balance
        shared_accounts[i].puddles_reward_rate = 0.02; // 2% flat reward rate

        // Create / Truncate savings file
        char filename[MAX_LINE];
        sprintf(filename, "savings/account_%d_output_savings.txt", i);
        shared_accounts[i].savings_file = fopen(filename, "w");
        if (shared_accounts[i].savings_file == NULL) 
        {
        ...
        // Write at top of file which account index it is
        fprintf(shared_accounts[i].savings_file, "account: %d\n", i);
    }
}
...
{
    // Update balances for all accounts in puddles bank
    for (int i = 0; i < num_accounts; i++) 
    {
        // Calculate the additional balance based on the 2% interest rate and balance
        double additional_balance = shared_accounts[i].puddles_reward_rate * shared_accounts[i].duck_bank_balance;

        // Update the puddles bank account balance
        shared_accounts[i].puddles_bank_balance += additional_balance;

        // Append each shared account's puddle balance to savings files
        fprintf(shared_accounts[i].savings_file, "Current Savings Balance \t%.2f\n", shared_accounts[i].puddles_bank_balance);
    }
}
...
void* process_update_balance(void* arg) 
{
    while (1) 
    {
        ...
        // Update Puddles Bank balances
        update_puddles_balances();

        // Reset the requests processed count
        requests_processed -= UPDATE_THRESHOLD;

        // Increment number of updates processed int
        num_updates++;
...
int main(int argc, char *argv[]) 
{
    ...
    // Create Puddles Bank process
    pid_t puddles_bank_pid = fork();

    if (puddles_bank_pid == 0) 
    {
        // Puddles Bank process
        // Read from shared memory and apply interest
        for (int i = 0; i < num_accounts; i++) 
        {
    ...
    else if (puddles_bank_pid > 0) 
    {
        // Create bank thread
        pthread_t bank_thread;

        // Initialize requests processed int
        requests_processed = 0;

        // Initialize number of updates processed int
        num_updates = 0;
    ...
    {
        // Handle fork error
        perror("Error creating Puddles Bank process");
        return EXIT_FAILURE;
    }

    // Wait for Puddles Bank process to finish
    waitpid(puddles_bank_pid, NULL, 0);

    // Print the final balances to the output file
    FILE* duck_bank_output_file = fopen("output/output_part_4_duck_bank.txt", "w");
    ...
    for (int i = 0; i < num_accounts; i++) 
    {
        fprintf(duck_bank_output_file, "%d balance:\t%.2f\n\n", i, accounts[i].balance);
    }

    // Close the duck bank output file
    fclose(duck_bank_output_file);

    FILE* puddles_bank_output_file = fopen("output/output_part_4_puddles_bank.txt", "w");
    ...
    for (int i = 0; i < num_accounts; i++) 
    {
        fprintf(puddles_bank_output_file, "%d balance:\t%.2f\n\n", i, shared_accounts[i].puddles_bank_balance);
    }

    // Close the puddles bank output file
    ...
    for (int i = 0; i < num_accounts; i++) {
        ...
        fclose(shared_accounts[i].savings_file);
    }
