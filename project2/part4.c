...
void print_status(const pid_t pid)
{
    ...
    // Format path
    snprintf(path, MAX_COMMAND_LENGTH, "/proc/%d/status", pid);

    // Open proccess status file from path
    ...
    // Iterate over every line in proccess status file
    while (getline(&line_buf, &len, process) != EOF) 
    {
        for (int i = 0; i < num_prints; i++) 
        {
            // Print line that I want to print
            if (strncmp(line_buf, process_prints[i], print_lengths[i]) == 0) 
            // 'strncmp' is a system call
            // However the return is used in the conditional below
            // If 'strncmp' gives an error it just goes to the next line
            // No need to return/exit
            {
                printf(" - %s", line_buf);
            }
        }
...
int schedule_next_process() 
{
    ...
    { 
        ...
        print_status(pid_array[current_process]);
    ...
    // Print relevant data for next process
    print_status(pid_array[current_process]);
...
int main(int argc, char *argv[]) 
{
    ...
    // Print relevant data for first process
    print_status(pid_array[current_process]);
